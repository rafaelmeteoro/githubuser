...
public class LeftFollowerAdapterDelegate implements AdapterDelegate<FollowersViewModelHolder> {
    ...
    private OnLeftFollowerClickListener listener;
    ...
    private void callOnFollowerClickIfNotNull(Follower follower) {
        if (listener != null) {
            listener.onClick(
                    new LeftFollowerClickData()
                            .url(follower.htmlUrl())
            );
        }
    }

    public void setOnFollowerClickListener(@Nullable OnLeftFollowerClickListener listener) {
